-- Function to notify users about new matches that match their ranking
CREATE OR REPLACE FUNCTION notify_users_of_new_match()
RETURNS TRIGGER AS $$
DECLARE
  user_record RECORD;
  user_ranking_value INTEGER;
  match_level_record RECORD;
  level_min INTEGER;
  level_max INTEGER;
...
BEGIN
  ...
  LOOP
    ...
    FOREACH match_level_record IN ARRAY NEW.match_levels
    LOOP
      -- Parse the match level to get min and max values
      CASE match_level_record::TEXT
        WHEN 'p50-p100' THEN
          level_min := 50;
          level_max := 100;
        WHEN 'p100-p200' THEN
